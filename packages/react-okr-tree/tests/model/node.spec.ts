...
import { TreeNode } from '../../src/model/node'
import { NODE_KEY, resetWarnings } from '../../src/model/util'
...
describe('TreeNode 基础构建', () => {
  ...
  /**
   * 源项目这条断言的是「实例是 shallowReactive 代理」。React 版没有代理层（R1），
   * 等价契约是：data 保持原始引用（否则 Q3/Q4 的引用比较失效）+
   * parent 与注册表持有的是同一个实例（否则赋值通知不到渲染方）+ 实例可订阅。
   */
  it('data 保持原始引用，parent / 注册表持有同一实例，且实例可订阅', () => {
    const data = makeData()
  ...
  it('状态字段赋值会通知订阅者，赋同值不通知（等值短路）', () => {
    ...
    const unsubscribe = a.subscribe(snapshot)
    const rev0 = a.getSnapshot()

    a.expanded = true
    expect(a.getSnapshot()).toBe(rev0 + 1)
    ...
    a.replaceChildNodes([])
    expect(a.getSnapshot()).toBe(rev0 + 2)
