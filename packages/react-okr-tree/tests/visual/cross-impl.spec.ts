...
/**
 * 跨实现 DOM 比对：react-okr-tree 与 vue3-okr-tree 渲出的 DOM 必须同形（G15 / vue3 侧 G9）。
 *
 ...
 * renderToStaticMarkup 现算，props 直接取 fixture 里那份，两边不会漂移。fixture 靠人工刷新：
 * - 它退化成本地快照，但锁的是**跨实现**形状，比只锁自己强；
 * - vue3 侧改了 DOM 契约而没刷新 fixture 时这里会红——先判断哪边是对的，再刷新。
 *
 ...
 *
 * 覆盖范围之外（不在本门禁里）：connector="svg" 的 d 值（要真实几何才有意义）、插槽与渲染函数
 * （两边 API 本就不同形）、懒加载 is-loading 与过渡中间态（SSR 拿不到）。
 */
