...
export function warnReadonlySource(action: string): void {
  warn(
 ...
 * 未配置 node-key 时，该标记作为列表 key 与 getNode(data) 的查找依据。
...
export const getNodeKey = function (key: string | undefined, data: TreeNodeData) {
  if (!key) return (data as any)[NODE_KEY] ?? nodeIdByData.get(data)
  return data[key]
}
...
/** 开发期警告（同一条默认只输出一次） */
export function warn(message: string, once = true): void {
  if (!isDev()) return
  if (once) {
    if (warned.has(message)) return
    warned.add(message)
  }
  console.warn(`[react-okr-tree] ${message}`)
}
