...
## 万级平铺（1 父 + 10000 平铺子节点，2026-09-26，不设门禁）

> 复现：`pnpm bench`（脚本末尾「万级」两条场景，jsdom）。数据形状与计时口径与上游 `vue3-okr-tree` 的 bench 同批同形，两条场景只差一个 `virtual`，差值即 DOM 渲染侧成本。

| 场景 | 耗时 | 渲染节点数 |
| ---- | ---- | ---------- |
| 首渲染 `virtual: false` | 978 ms | 10001 |
| 首渲染 `virtual: true` | 111 ms | 13 |
| 差值 = 渲染与布局侧 | 867 ms | — |

- 与上游同一条结论：`virtual` 把渲染节点压掉 99.87%，首帧只降 89%；剩下那 111 ms 是挂载期的模型侧成本。本包 `data` 是普通对象、模型层不包代理（见「三条结构性差异」第 1 条），所以这 111 ms 基本就是 10001 个模型实例的构造 + 注册。
- **两仓的关键差在这里**：同形状上游实测是 10953 ms / 5494 ms（其 `docs/perf.md` 同名一节），**约 11 倍 / 50 倍**。上游已用自己的 bench 排除掉两项嫌疑（摘掉 `deep-watch`、`data` 不套 `reactive`，两次都在 4.9–5.6 s 的噪声内），剩下的嫌疑落在框架侧的每节点构造开销上。**这是上游侧的待查项，本包不跟进**（两边模型层同源，本包这条路径没有对应的开销可查）。
- 不设门禁：万级没有验收线，`tests/visual/perf.spec.ts` 守的是既有的「2040 节点 Chromium 首渲染」那条。
