...
`react-okr-tree` 是 `vue3-okr-tree` 的 React 移植：props、回调与 ref 方法与上游**一一对应**（条数见 [API](/docs/api)），默认值、抛错文案与时机、DOM 类名与结构、两份样式表全部逐字对齐。差异只来自框架范式，共 12 条，编号 D1–D12，全部列在本页。
