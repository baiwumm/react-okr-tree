...
| 依赖                  | 要求                 | 说明                                                                                                                                                                                                                                                                                                           |
| --------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `react` / `react-dom` | `>= 18.2.0`（peer）  | 用到的能力（`useSyncExternalStore` / `forwardRef` / `useImperativeHandle` / Context）自 18.2 起齐备；CI 的 peer 矩阵在 `~18.2.0` 与 `~18.3.0` 两档各跑一遍全量单测（`@types/react` 同步降到 18，以便让 React 19 独有的类型依赖在 18 下编译失败被捕获），React 19 由 workspace 默认安装的那条 `verify` 作业覆盖 |
...
ref 拿到的是 `OkrTreeHandle`，命令式方法都在这里（完整表见 [Methods](/docs/api)）：
