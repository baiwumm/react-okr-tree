...
- **`node-drag-end` 在成功放置后不再报 null 载荷**：`handleDrop` 先清掉 `dragOver`（指示线不能留在屏幕上），而 `dragend` 是在 drop 之后才派发的，于是这条事件的 `dropNode` / `dropType` 恒为 null——文档站的拖拽用例每次成功拖动都记一句「未完成放置」。更糟的是本包跨父级移动会把源元素卸载重建（React 的键只在同一父级内去重），浏览器那次 `dragend` 落在一个已脱离文档的节点上，**宿主侧根本收不到这条事件**。现在统一为**在 drop 里就地发出 `node-drag-end` 并收尾手势**（同时清 `draggingNode`，让真到达的那次 dragend 被 `getDraggingNode() !== node` 守卫短路，不会补一条重复事件），载荷与 `node-drop` 一致；取消或被拒的拖拽仍按语义报 null。与上游 `vue3-okr-tree` 同批同形。
- **过滤词只命中 OKR 左子树时，整棵树不再从 DOM 消失**：共用的根节点自身不匹配、右树侧零命中时被判不可见，而节点根元素是 `if (!node.visible) return null`——刚命中的左子树跟着被卸载，页面空掉（`getVisibleNodes()` 读 0）。Q1「父节点保持可见」当年只修通了右树这一侧；现在左树命中也会把根保住（OKR 下 `filter` 先右后左的调用顺序是承重的，已写进注释）。
- **`defaultCheckedKeys` 运行时按内容比较，不再按引用重放**：宿主每次渲染新建一个等值数组（行内字面量、漏了 `useMemo` 的派生表达式都会这样），原先被当成「默认勾选变了」，先清空全部勾选再重放列表，把用户刚勾掉的、刚勾上的整片抹回去。现在只有列表**内容**真的变了才重放（`String(key)` 归一去重后比对，与 `getCheckedKeys` 同口径），`undefined` 与 `[]` 视作同一份「没有默认勾选」。API 表与文档站措辞同步更正；文档站 checkbox 用例此前因行内字面量而整页勾选点不动，已随该批把常量提到模块作用域。
- **画布平移把手势甩出画布边界再松手，会卡住平移态并让悬停继续拖动画布**：`OkrTreeViewport` 的 `onPointerUp` / `onPointerCancel` 只挂在画布根元素上（既没有 `setPointerCapture`，也没有 window 级监听），指针在画布外松开时这两个处理器根本不执行——`is-panning` 留在原地、`panStart` 也不清，于是之后**不带按键**的悬停移动仍按 `panStart` 继续改写偏移。现在由一个挂载期注册、卸载时摘掉的常驻 window 监听收尾（常驻而不是按次添加，免得 1.14.1 修掉的监听堆叠换个形式回来）；这条收尾**不**武装「吞掉一次 click」——松手在画布外时浏览器不会在画布里派发那次 click，武装了就会吃掉用户回到画布里的第一次正常点击。与上游 `vue3-okr-tree` 同批同形。

## 1.14.1（2026-09-24）

跟随上游 `vue3-okr-tree` 1.14.1 同号发布（锁步约定）。**对外 API 零变化**：内容全部是上游那批缺陷修复的同号移植（8 条：组对齐宽度被首量钉死、`contains` 未递归左子树可成环、换绑 `data` 把整棵后代摘出注册表、左树顶层结构性增删脱钩、挂载期白建一次左树、运行中改 `animate` 撑高已折叠容器、平移吞点击堆监听、产物丢失三家打包器的 ignore 注释）、3 项性能收敛、10 条门禁补强与文档站更正。
...
- **缺 node-key 的受控 prop 警告族补齐三条断言（G4）**：`src/OkrTree.tsx:612-620` 有四条「需要同时设置 node-key」的开发期警告，`controlled.spec.tsx` 此前只在一次 pooled 断言里命中了 `expanded-keys` 与 `current-key` 两条文案，`default-expanded-keys` / `default-checked-keys` 两支**从未被断过**，`currentNodeKey` 那一支也只是**顺带过的**——它的消息文案写着「current-key / currentNodeKey」，所以只传 `currentNodeKey` 时靠子串也能命中，把源码里的 `|| p.currentNodeKey !== undefined` 摘掉都不会红。现新增一条用例：三个 prop 各单独挂载一次、每次都断「这一次的消息集合」，另加一条反向守卫（补齐 node-key 后三条都不许出现）。反向守卫一开始是**永真**的：`warn()` 默认按文案去重，第四次挂载前没清表，消息被前三次吃掉，实测把 `if (!p.nodeKey)` 改成无条件警告后这条守卫照样绿（红的只是文件里另一条老用例）——现每次挂载前 `resetWarnings()`，改无条件后两条用例一起红。三条变异（摘 default-expanded-keys 分支 / 摘 currentNodeKey 半个条件 / 去掉 node-key 守卫）跑两仓各一遍，全部只红新用例；驱动脚本 `_scratch/probe-warn-mutation.mjs`（含未变异基线与「红必须是断言失败而非编译错误」两道自检，finally 还原并核对 `git status`）。上游 `vue3-okr-tree` 同批同形。单测 278 → **279** 条。
- **受控锁定态补用例（G8），并实测纠正了这条缺口原本的判据**：requirements 第 5 节与第 1 轮报告都把「传值但不传回调」写成一种**锁定**，报告给的判据是「点 +/- 后断言视图不变」。**照写会得到一条红用例**——实测两仓都不冻结视图：展开态的渲染源是 store，受控 prop 只在创建期（`:293`）与宿主传入值变化时（`:832-837`）回灌，宿主不回写就没有第二次同步，点击照常折叠（上游 vue3 侧同样三种写法逐字测过，结论一致，所以这不是复刻偏差）。现按真实语义钉两条：① 不传回调 → 不报错不警告、点击生效；② 传了回调但宿主不回写 → 回调照样收到新值（收起根节点即 `[]`）、视图不被拉回。三条变异验证判据有牙：摘掉 emit 红 ②；在交互回调里以 prop 为准拉回（＝真冻结）只红这两条；去掉 node-key 守卫让锁定态也警告红 ①。**第三条一开始是白断的**：`warn()` 按文案去重、去重表还是模块级的，本文件前面的用例已经把那条文案发掉，不自己 `resetWarnings()` 的话 `not.toHaveBeenCalled()` 恒真（实测改无条件警告时它照样绿）。**仍未验的一条**：`guide/controlled.mdx:91-93` 声称「传内联字面量才完全锁定、引用稳定则保留交互结果」，探针没能分离出这个差别——宿主重渲染时 `data={makeData()}` 同时也换了引用、触发整树重建，两条路径混在一起；要证得先给宿主一份引用稳定的 data 再对比，已记待办（不改文档也不改实现，等结论）。上游同批同形。单测 279 → **281** 条。
- **拖拽悬停的渲染计数判据（收口 G9）**：`setDragOver`（`src/OkrTree.tsx:483-489`）本就是「只 bump 新旧两个节点」的局部通知，但这句话此前只有实现、没有断言——`draggable.spec.tsx` 的 11 条功能用例断的是「放不放得下、源数据有没有同步」，把 `if (prev.node) prev.node.notify()` 摘掉、或干脆换成整树 `forEachNode(n => n.notify())`，全套用例照样绿。现按 `local-update.spec.tsx` 键盘漫游那条的形状补一条，按事件顺序逐个断谁被 bump：dragstart 零 bump（`dragging` 只写在 ref 上，没有渲染状态读它）→ 首次悬停只 bump 被悬停节点 → 同节点同分区重复 dragover 零 bump（`setDragOver` 的同值早退）→ 换目标恰好两个（旧撤下 + 新挂上）→ dragEnter 零 bump（只转发事件）。每步同时断对应 label 上的 `drop-*` 类名，避免「指示根本没挂上」造成的空断。五条变异逐个跑过、全部只红这条（驱动 `_scratch/probe-drag-mutation-react.mjs`，自带基线绿跑、锚点唯一性自检与 finally 还原）。上游 `vue3-okr-tree` 同批补同名文件的对称判据，**但落点是 DOM 写入而不是版本 bump**：vue3 用 provide 下来的 `shallowRef` 承载拖拽指示，实测换一次目标时 18 个节点的 `labelWrapperClass` 全部重新求值、只有真变化的 2 个元素被写入 DOM——机制不同，故判据各按各的形状写，不是复刻偏差。单测 281 → **282** 条。
- **OKR 根卡片「展开 / 收起不被推位」补几何断言（收口 G14）**：`align-root` 的全部作用就是这一句，此前只有那条 CSS、`align-root` 类名与一张展开态的像素基线，收起这个动作序列本身没有任何几何断言守着。`visual.spec.ts` 新增一条：对 `#demo-okr-group` 第一棵树的根卡片量 `getBoundingClientRect()`，右子树收起 → 重新展开 → 左子树收起 → 重新展开，四次 `toBeCloseTo(x, 0)` + 两次宽度比对；每步同时断 `-children` / `-left-children` 上 `is-hidden` 的确切出现与消失，否则「坐标没变」可能只是因为按钮没生效。**量的是相对 `.org-chart-container` 的水平偏移而不是绝对 `left`**——demo 卡在 `overflow-x-auto` 里，Playwright 点击前会把目标滚进视口，实测第一次与第二次读取之间绝对 `left` 从 962 跳到 591，那是滚动位置不是布局，用绝对坐标只会得到一条与几何无关的红。**只加断言、没加截图基线**（png 数不变）。四条变异逐个跑过，射程按实测记：给 `.is-hidden` 追加 `display:none`（收起即从布局消失）→ 本条红、像素基线同样红；`flex:1 1 0` → `0 0 auto` 与根节点 `width:100%` → `auto` 两条 → 本条与 `okr-group-aligned` 基线**都绿**，是 `group-align.spec.ts` 把它们打红的；`min-width:max-content` → `0` 由像素基线单独抓住。所以这条守的是「收起不推根节点」，不是 `align-root` 的静态形状——写文档时别把它说大。变异改的是 `out/` 里那份打包 CSS（gitignored 产物，跑完逐字还原）：浏览器读的就是它，改 `src/styles/style.css` 得重跑两次构建（约 4 分钟一条）。上游 `vue3-okr-tree` 同批同形（其 `demo-10`），四条门禁归属逐条复测一致。视觉用例 33 → **34** 条。
- **焦点环补计算样式断言（G13 里不依赖像素的那一半）**：环此前只有类名与人眼看图，`tests/visual/` 内 grep `focus` 零命中。新增一条：`page.keyboard.press('Tab')` 循环走到第一个 `.org-chart-node`（上限 80 次，走不到即判失败——否则后面的环断言全在空转），再断该节点卡片本体的计算 `outlineStyle='solid'`、`outlineWidth='2px'`、颜色非透明，并顺带断焦点元素自身 `outlineStyle='none'`（`:focus{outline:none}` 那条重置的现状）。**差点照直觉写错**：报告和常规写法都会去读「聚焦元素」的 outline，探针实测聚焦元素本身算出来是 `none`——真正的环挂在 `.org-chart-node:focus-visible > .org-chart-node-label > .org-chart-node-label-inner`，读错元素直接得到一条假红。四条变异两仓各跑一遍、全部打红：整条改 `outline:none`；环宽兜底 `2px→0px`（两份产物都没定义 `--okr-focus-width`，兜底就是实际生效值，改它确实改渲染）；**去掉 `:focus-visible` 限定让环常驻**——这条专门反自证，前三条断言它照样全绿，只有「Tab 走开之后环必须收掉」那条抓得住它；把环挪回焦点元素自己身上（同时打红「环在卡片上」与「节点自身是 none」两条）。变异改 `out/` 里的打包 CSS（gitignored 产物，跑完逐字还原）。G13 剩下的 `orthogonal` / `straight` 两种 connector 形状是三张像素基线的事，不在本批范围。视觉用例 34 → **35** 条，png 仍 30 张。上游 `vue3-okr-tree` 同批同形（其 `demo-1`），四条变异复测一致。
- **两种锁定强度实测分离并补用例（把 `guide/controlled.mdx` 那句说法钉住）**：文档站受控页写着「`expandedKeys` 写成每次渲染新建的数组字面量 → 完全锁定；引用稳定的数组 → 交互结果保留到 prop 真的变化为止」，第 2 批想验没验成——宿主重渲染时 `data={makeData()}` 同时也换了引用，走整树重建那条路，两条分支的观测结果逐字相同（正是被整树重建主导的那种「相同」）。这批把 `data` 提成 `useMemo` 稳定引用后重测，**说法成立**：内联字面量 ⇒ 点完收起、宿主一重渲染就被拉回展开；引用稳定 ⇒ 折叠结果保留，且**同一数组原地 push 不算变化**（`:832-837` 那条 effect 的依赖按 `Object.is` 比），换成新引用才回灌。新增两条用例逐档钉住。两条变异打红：摘掉那条 effect（两条新用例一起红，原有两条「不回写即不冻结」的用例保持绿，归属清楚）；把依赖改成 `[...expandedKeys]` 按内容比（原地 push 就生效，红）。**一条变异作废并记在这里**：原本想用「去掉依赖数组」模拟永远锁定，实测变成 `Maximum update depth exceeded`——effect 里 `bumpAll()` 又触发渲染，红法是崩溃不是断言，不能作为证据，故弃用。上游 vue3 同批测同三条分支，逐字一致；vue3 那边多一种情形（数组是 `ref` 包着的响应式对象时，原地变更会被它的 `deep: true` watcher 收到），本包无对应机制、不硬造。另记一条机制差：vue3 侧「引用稳定 ⇒ 保留」是**结构性**的（prop 引用没变时子组件根本不被更新，库内构造不出打红它的变异），本包则真有对应变异（见上）。单测 282 → **284** 条。
