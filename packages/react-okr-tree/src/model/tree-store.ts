...
export class TreeStore {
  ...
  /**
   * 突变广播（requirements R1 第 6/7 条）：任意节点的状态变化都会打一发。
   *
   * 只给命令式消费者用（SVG 连接线的重绘调度）。React 组件一律订阅自己那个节点，
   * 谁要是拿它去驱动整树重渲染，R1 的局部更新就白做了。
   */
  private mutation = new Notifier()
  ...
  subscribeMutation = this.mutation.subscribe
  ...
  filterRight(value: any, childName: ChildName) {
    ...
    this.root.childNodes.forEach(rootNode => {
      ...
      if (childName === 'childNodes') {
        ...
        // 左树命中时先把共用的根节点保住可见：根一被判不可见，节点根元素的
        // `if (!node.visible) return null` 会把整棵树（含刚命中的左子树）一起卸载，页面直接空掉。
        // 顺序是承重的：OKR 下 filter 先右后左（见 OkrTree 的 filter），
  ...
  getNode(data: TreeNode | TreeKey | TreeNodeData | null | undefined): TreeNode | null {
    ...
    const key = typeof data !== 'object' ? data : getNodeKey(this.key, data as TreeNodeData)
   ...
   * 门控与节点组件的渲染分支一致：右节点的 childNodes 看 expanded，
   ...
   * 比的是**内容**不是引用：宿主每次渲染新建一个等值数组（React 的行内字面量、
   * `useMemo` 漏掉的派生表达式都会这样）不该被当成「默认勾选变了」——那会把用户刚勾掉、
   * 刚勾上的状态整片抹回去。undefined 与 [] 视作同一份「没有默认勾选」。
   */
