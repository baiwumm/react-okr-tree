/**
 * 构建后处理（移植自源项目 scripts/post-build.mjs）：
 ...
 * 2. 压缩 ESM 产物。Vite lib 多格式构建中 es 输出不经过压缩
 *    （cjs/umd 正常压缩，es 带完整缩进与换行），用构建链内置的 esbuild 补一次压缩，
 *    使三种格式体积同量级。
 */
...
const esFile = resolve(root, 'dist/react-okr-tree.es.js')
if (existsSync(esFile)) {
  ...
  // 压缩后与构建期 sourcemap 错位（transform 无法串联既有 map），删除过期 map
  ...
  console.log(
    `[post-build] 已压缩 dist/react-okr-tree.es.js：${kb(before)} → ${kb(after)}（gzip ${kb(gzip)}），已移除错位的 es.js.map`
  )
}
